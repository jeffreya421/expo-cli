...
import { Project, ProjectUtils, UserManager, Versions, TurtleConfig, TurtleApi, FormData } from '@expo/xdl';
...
export default class BaseBuilder {
  ...
  async postBuild(projectDir: string, options: Options) {
    const tarName = `${uuidv4()}-project.tgz`;
    const tarPath = `tmp/${tarName}`;
    await this.getProjectTarball(projectDir, tarPath);
    const projectTarFile = new FormData();
    projectTarFile.append('file', `tmp/${tarName}`);
    const s3Url = await TurtleApi.clientForUser().uploadProject(projectTarFile);
    const data = this.getUserData(options, s3Url);
    await TurtleApi.clientForUser().postAsync('build/start', data);
  }
  ...
  async getProjectTarball(projectPath: string, tarPath: string) {
    await tar.create({
      gzip: true,
      file: tarPath,
    },
